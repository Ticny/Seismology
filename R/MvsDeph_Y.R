...
#' @description This function creates a graph of Month vs Depth for a specific year from the seismic catalog, with points that represent the magnitude_value_P
...
#'
Depsism_Y<-function(catalogYM, Y){
  PSism_Y<-catalogYM %>% filter(year==Y)
  ...
  Graf<-ggplot(PSism_Y, aes(x=month, y=depth_value, size=magnitude_value_P, color=magnitude_value_P))+
    ...
    labs(title=paste("Depth and magnitude of seismic events in the year", Y),x="Months",y="Depth")
  return(Graf)
}
