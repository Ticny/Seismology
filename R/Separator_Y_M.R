...
#' @description Using this function, we can extract the Year and Month information from a seismic catalog for a more detailed analysis. This function works with seismic catalog data where the time_value variable is expressed in the format Year-Month-Day and UTC Time.
...
#' SepYM(catalog)
#'
SepYM<-function(catalog){
  catalog$time_value<-as.Date(catalog$time_value, "%Y-%m-%d",  tz="UTC")
  catalog$year<- format(catalog$time_value, "%Y")
  catalog$month<-format(catalog$time_value, "%m")
  return(catalog)
}
