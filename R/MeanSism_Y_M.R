#' Mean seismic magnitude for year and month
#' @description With this function, we can calculate the mean seismic magnitude for a specific year and month.Generally, seismic events with low magnitudes are more common than those with higher magnitudes. Therefore, the mean magnitude value typically ranges from 3 to 4. Large differences in the mean magnitude value may indicate issues with the catalog or calculations.
#' @details Catalog YM: The catalog with the year and the month separated
#' @details Y: Year that needs to be analyzed
#' @details M: Month that needs to be analyzed
#'
#' @param catalogYM
#' @param Y
#' @param M
#'
#' @return mean magnitude for year and month
#' @export
#' @import magrittr
#' @import dplyr
#'
...
#' MeanS4YM(catalogYM, 2012,02)
#' MeanS4YM(catalogYM, 2012,03)
#'
MeanS4YM<-function(catalogYM, Y, M){
  Mean_YM<- catalogYM %>% filter(year==Y) %>% filter (month==M) %>%
